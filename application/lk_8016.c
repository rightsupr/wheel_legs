...
static void lk8016_register(struct Lk8016 *motor) {
...
void lk8016_can_msg_unpack(uint32_t id, uint8_t data[]) {
  ...
  switch (id) {
    case JOINT_PF_RECEIVE: {
      ...
      shoulder[0].angular_vel = speed_int * PI / 180;
      motors[0]->torque = iq_int/16.535;
      shoulder[0].torque = iq_int/16.535;
      motors[0]->encoder = encoder_int/182.04;
      shoulder[0].encoder = encoder_int/182.04;
      break;
    }

    case JOINT_PB_RECEIVE: {
      speed_int = (int16_t) ((data)[5] << 8 | (data)[4]);
      iq_int = (int16_t) ((data)[3] << 8 | (data)[2]);
      encoder_int = (int16_t) ((data)[7] << 8 | (data)[6]);
      motors[1]->angular_vel = speed_int * PI / 180;
      shoulder[1].angular_vel = speed_int * PI / 180;
      motors[1]->torque = iq_int/16.535;
      shoulder[1].torque = iq_int/16.535;
      motors[1]->encoder = encoder_int/182.04;
      shoulder[1].encoder = encoder_int/182.04;
      break;
    }
    default:break;
  }
...
void lk8016_encoder_read_back(uint32_t id, uint8_t data[]) {
  ...
  switch (id) {
    case JOINT_LF_RECEIVE: {
      ...
      shoulder[0].encoder = encoder_int/600.0;
      break;
    }

    case JOINT_LB_RECEIVE: {
      encoder_int = (uint32_t)((data)[7] << 24 |(data)[6] << 16 | (data)[5] << 8 | (data)[4]);
      shoulder[1].encoder = encoder_int/600.0;
      break;
    }
    default:break;
  }
